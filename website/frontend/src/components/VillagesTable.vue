...
const props = defineProps({villagesArr: Array});
...
<!---------------------------------------------------->
<template>
<div id="villages-table" class="concept-table">
    <DataTable :value="villagesArr" :paginator="true" class="p-datatable-sm" showGridlines :rows="10" rowHover dataKey="id" v-model:filters="filters" filterDisplay="menu" :loading="loading" paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown" :rowsPerPageOptions="[10,25,50]" responsiveLayout="scroll" :globalFilterFields="['id','name','culture','village_type']">
        <template #header>
        ...
        <Column field="culture" header="Culture" sortable :showFilterMatchModes="false" :filterMenuStyle="{'width':'14rem'}">
            <template #body="{data}">
                {{data.culture}}
            </template>
        ...
        <Column field="village_type" header="Village Type" sortable>
            <template #body="{data}">
                {{data.village_type}}
            </template>
            <template #filter="{filterModel, filterCallback}">
                <InputText type="text" v-model="filterModel.value" @input="filterCallback()" class="p-column-filter" :placeholder="`Search by Village Type - `"/>
            </template>
        </Column>
        <Column field="x_position" header="X-Position" sortable></Column>
        <Column field="y_position" header="Y-Position" sortable></Column>
        <Column field="hearth" header="Hearth" dataType="numeric" sortable>
            <template #body="{data}">
                {{data.hearth}}
            </template>
            <template #filter="{filterModel}">
        ...
        <Column field="bound_settlement" header="Bound Town/Castle"></Column>
    </DataTable>
</div>
</template>
<!---------------------------------------------------->
