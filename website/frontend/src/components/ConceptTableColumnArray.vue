...
<!------------------------------------------------------->
<template >
    <router-link :to="{name: 'cardview', params: {concept: getConceptFromObject(header, fieldArrObj.id), id: fieldArrObj.id}}" class="id-link">
        {{ fieldArrObj.name }}
    </router-link>
    <span v-if="isLastObject(props.fieldArr, props.fieldArrObj)">, </span>
</template>
<!------------------------------------------------------->
<style scoped>
